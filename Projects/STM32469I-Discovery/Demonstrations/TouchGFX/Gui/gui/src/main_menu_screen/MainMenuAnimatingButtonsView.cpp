...
void MainMenuAnimatingButtonsView::setupScreen()
{
    ...
    demoIDs[3] = GRAPH;
    icons[3].setBitmap( Bitmap( BITMAP_ANIMATING_BUTTONS_MENU_ICONS_GRAPH_TINY_ID ) );
    texts[3].setTypedText( TypedText( T_ANIMATING_BUTTON_GRAPH_HEADLINE ) );
    texts[3].setMoveAnimationEndedAction( textAnimationDone );
...
{
    if( currentAnimationState == ANIMATE_ICONS_AND_TEXT_IN && &t == &texts[3] )
    {
        currentAnimationState = NO_ANIMATION;
    }
}
