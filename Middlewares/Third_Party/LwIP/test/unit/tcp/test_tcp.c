...
/** Call tcp_new() and tcp_abort() and test memp stats */ START_TEST( test_tcp_listen_passive_open )
{
 ...
 * overlapping rcv_nxt is received */ START_TEST( test_tcp_recv_inseq_trim )
{
    struct test_tcp_counters counters;
    struct tcp_pcb *pcb;
    struct pbuf *p;
 ...
 * that already has been sent before. */ START_TEST( test_tcp_retx_add_to_sent )
{
    struct netif netif;
    struct test_tcp_txcounters txcounters;
    struct test_tcp_counters counters;
